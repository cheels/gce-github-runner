...
function start_vm {
  ...
  startup_script="
    ...
    RUNNER_ALLOW_RUNASROOT="nonroot" ./config.sh --url https://github.com/${GITHUB_REPOSITORY} --token ${RUNNER_TOKEN} --labels ${VM_ID} --unattended ${ephemeral_flag} --disableupdate && \\
    ./run.sh && \\
